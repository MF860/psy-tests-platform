...
namespace PsyApi.Controllers
{
    ...
    public class AdminAiController : ControllerBase
    {
        ...
        public AdminAiController(AppDbContext db, IAiAnalyzerService ai, IAuditService audit)
        {
            _db = db; _ai = ai; _audit = audit;
        }
        ...
        public async Task<IActionResult> Analyze([FromBody] AnalyzeRequest request)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        var parsed = JsonSerializer.Deserialize<List<PsyApi.Models.DimensionScore>>(entity.DimensionScoresJson, JsonOptions);
                        if (parsed != null) dims = parsed;
                    }
                }
                catch (Exception ex)
                {
                    // Log dimension parsing error but continue with empty list
                    Console.WriteLine($"[AI] Failed to parse dimensions for result {request.ResultId}: {ex.Message}");
                }

                var response = await _ai.AnalyzeAsync(entity, dims);

                await _audit.LogAsync(null, "AI_ANALYZE", $"ResultId={entity.Id};SessionId={entity.SessionId};User={entity.Session.User.NationalId}", HttpContext.Connection.RemoteIpAddress?.ToString());
