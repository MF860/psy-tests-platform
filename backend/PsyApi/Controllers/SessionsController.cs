...
namespace PsyApi.Controllers
{
    ...
    {
        new QuestionOption("لا أوافق بشدة", "لا أوافق بشدة"),
        new QuestionOption("لا أوافق", "لا أوافق"),
        new QuestionOption("محايد", "محايد"),
        new QuestionOption("أوافق", "أوافق"),
        new QuestionOption("أوافق بشدة", "أوافق بشدة")
    };

    private static readonly IReadOnlyList<QuestionOption> FrequencyOptions = new List<QuestionOption>
    {
        new QuestionOption("أبدًا", "أبدًا"),
        new QuestionOption("نادرًا", "نادرًا"),
        new QuestionOption("أحيانًا", "أحيانًا"),
        new QuestionOption("غالبًا", "غالبًا"),
        new QuestionOption("دائمًا", "دائمًا")
        ...
        public async Task<IActionResult> StartSession([FromBody] Models.StartSessionRequest request)
        {
            try
            {
                ...
                // Randomly select items (configurable, default 80)
            ...
            try
            {
                ...
                if (!TryParseItemId(request.ItemId, out var itemCode))
                {
                    return BadRequest(new { 
                        error = "رقم السؤال غير صالح. يجب أن يكون بصيغة I001", // Invalid Item ID format. Must be in format I001
        ...
        private static bool TryParseItemId(string? raw, out string itemCode)
        {
            ...
            // Only accept I prefix with exactly 3 digits
            if (trimmed.StartsWith("I", StringComparison.OrdinalIgnoreCase) && trimmed.Length == 4)
            {
                var numericPart = trimmed.Substring(1);
                if (int.TryParse(numericPart, out var itemId) && itemId > 0 && itemId <= 999)
                {
                    itemCode = $"I{itemId:D3}";
                    return true;
                }
            }
