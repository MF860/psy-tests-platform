...
{
    /// <summary>
    /// OpenRouter API client for DeepSeek integration
    /// </summary>
    ...
    public class OpenRouterClient : IOpenRouterClient
    {
            ...
            ILogger<OpenRouterClient> logger)
        {
            ...
            if (!string.IsNullOrEmpty(_config.HttpReferer))
                _httpClient.DefaultRequestHeaders.Add("HTTP-Referer", _config.HttpReferer);
            if (!string.IsNullOrEmpty(_config.XTitle))
                _httpClient.DefaultRequestHeaders.Add("X-Title", _config.XTitle);
        }
