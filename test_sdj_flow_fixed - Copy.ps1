...
# This script tests the entire SDJ session flow: start â†’ answer all questions â†’ submit â†’ verify sdjData
...
Write-Host "  âœ“ Session ID: $sessionId" -ForegroundColor Green
Write-Host "  âœ“ Total Questions: $totalQuestions" -ForegroundColor Green
Write-Host ""
...
for ($i = 0; $i -lt $totalQuestions; $i++) {
    try {
        ...
        Write-Host "  âœ— Error at question $($i+1): $($_.Exception.Message)" -ForegroundColor Red
        break
    }
}

Write-Host "  âœ“ Answered $answeredCount questions" -ForegroundColor Green
Write-Host ""
...
try {
    ...
    Write-Host "  âœ“ Session submitted successfully" -ForegroundColor Green
    
    # Extract result ID from response if available
    if ($submitResponse.resultId) {
        $resultId = $submitResponse.resultId
        Write-Host "  âœ“ Result ID: $resultId" -ForegroundColor Green
    }
}
catch {
    Write-Host "  âœ— Submit failed: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host ""
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        Write-Host "  âœ“ Result saved to submit_result.json" -ForegroundColor Green
        
        # Verify sdjData presence
        if ($result.sdjData) {
            Write-Host "  âœ“ sdjData present in result" -ForegroundColor Green
            
            # Check for SubDimensions
            if ($result.sdjData.SubDimensions) {
                $subdimensionCount = $result.sdjData.SubDimensions.Count
                Write-Host "  âœ“ Found $subdimensionCount subdimensions in sdjData" -ForegroundColor Green
            ...
            Write-Host "  âœ— sdjData NOT found in result (this indicates legacy mode or scoring issue)" -ForegroundColor Red
        }
    }
    else {
        Write-Host "  âœ— No results found" -ForegroundColor Red
    }
}
catch {
    Write-Host "  âœ— Failed to retrieve result: $($_.Exception.Message)" -ForegroundColor Red
}
